...
async def on_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data == "to_menu":
        ...
        b_id = db_save_booking(user_id, context.user_data.get("b_service"), context.user_data.get("b_date"), context.user_data.get("b_time"), context.user_data.get("b_comment", ""))
        await safe_send(update, context, "✅ Запись создана! Ожидайте подтверждения мастера.")
        user = db_get_user(user_id)
        admin_text = f"<b>🆕 Новая запись!</b>\n\n👤 Клиент: {user[1]} ({user[2]})\n💅 Услуга: {context.user_data.get('b_service')}\n📅 Дата: {context.user_data.get('b_date')}\n⏰ Время: {context.user_data.get('b_time')}\n"
        if context.user_data.get("b_comment"): admin_text += f"💬 Комментарий: {context.user_data.get('b_comment')}\n"
        kb = InlineKeyboardMarkup([[InlineKeyboardButton("✅ Подтвердить", callback_data=f"adm_conf_{b_id}"), InlineKeyboardButton("❌ Отклонить", callback_data=f"adm_rejc_{b_id}")], [InlineKeyboardButton("💬 Написать клиенту", callback_data=f"adm_msg_{user_id}")]])
        ...
        await context.bot.send_message(chat_id=booking[1], text=f"✅ Ваша запись на <b>{booking[3]} {booking[4]}</b> подтверждена мастером!", parse_mode="HTML")
    elif data.startswith("adm_rejc_"):
        b_id = int(data.split("_")[2])
        db_update_booking_status(b_id, "cancelled")
        booking = db_get_booking(b_id)
        await query.edit_message_text(query.message.text + "\n\n❌ ОТКЛОНЕНО")
        await context.bot.send_message(chat_id=booking[1], text=f"❌ К сожалению, мастер не может принять вас <b>{booking[3]} {booking[4]}</b>.", parse_mode="HTML")
...
async def reminder_job(context: ContextTypes.DEFAULT_TYPE):
    ...
    for b in bookings:
        try:
            text = f"🔔 <b>Напоминание о записи завтра!</b>\n\n💅 Услуга: {b[2]}\n⏰ Время: {b[4]}\n🏠 Адрес: {ADDRESS_TEXT}\n🔗 <a href='{MAPS_URL}'>Открыть на картах</a>\n\nДо встречи! 💛"
        ...
        except Exception as e: logger.error(f"Failed to send reminder: {e}")
    conn.commit()
    conn.close()
...
def main():
    ...
    application.job_queue.run_repeating(reminder_job, interval=600, first=10)
